...
%         if c(i) + mu > best_c_val_og  % THIS WORKS
%         if c(i) + mu > best_c_val  % THIS WORKS
            best_c_idx = i;
            best_c_val = c(i);
...
% new_kernel_weight = sum(bitand(zm(new_kernel,:), not(z)))/sum(not(zm(new_kernel,:)));
% curr_kernel_weight = sum(bitand(z, not(zm(new_kernel,:))))/sum(not(zm(new_kernel,:)));
